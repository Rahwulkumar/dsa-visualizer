...
const MemoryVisualization = ({ stack = [], top = -1, maxSize = 8, codeLanguage }) => {
  const baseAddress = 0x7fff5fbff000;
  const rowHeight = 40; // Made constant for calculation, but can be CSS var if needed
  ...
  return (
    <div 
      className="col-span-4 flex flex-col glass-card p-6 h-full bg-gradient-to-br from-gray-900/90 to-purple-900/90 backdrop-blur-xl border border-gray-700/50 md:grid md:grid-cols-2 md:gap-6"
      role="region"
    ...
    >
      ...
      <div className="flex flex-col">
        ...
        <div 
          className="flex-grow bg-black/30 p-3 rounded-lg border border-white/10 relative scrollbar-thin"
          role="table"
          aria-label="Stack memory addresses and values"
        >
          <div className="flex flex-col gap-1" role="rowgroup" style={{ '--row-height': `${rowHeight}px` }}>
            <AnimatePresence>
              {stackMemory.slice().reverse().map((mem, displayIndex) => {
                ...
                return (
                  <motion.div
                    ...
                    exit={{ opacity: 0, x: 20 }}
                    className={`flex items-center justify-between p-2 rounded-md transition-all duration-300 height-[var(--row-height)] ${
          ...
          {top !== -1 && (
            <motion.div 
              layout
              initial={{ y: rowHeight * (maxSize - top - 1) }}
              animate={{ y: rowHeight * (maxSize - top - 1) }}
              transition={{ type: 'spring', stiffness: 400, damping: 30 }}
              className="absolute -right-16 flex items-center gap-2"
              style={{ top: `${12 + rowHeight * (maxSize - top - 1)}px` }}
