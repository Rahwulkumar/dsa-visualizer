...
const QueueControls = ({
  ...
  const operations = [
    { id: 'enqueue', label: 'Enqueue', icon: Plus, color: 'from-green-500 to-green-600' },
    { id: 'dequeue', label: 'Dequeue', icon: Minus, color: 'from-red-500 to-red-600' },
    { id: 'peek', label: 'Peek/Front', icon: Eye, color: 'from-cyan-500 to-cyan-600' },
    { id: 'display', label: 'Display Queue', icon: Shuffle, color: 'from-purple-500 to-purple-600' }
  ];
  ...
  const getPreviewElementStyle = (index) => {
    ...
    // Operation states override queue position states
    switch (state) {
      case 'enqueuing':
        className = 'bg-green-600/90 border-green-400 text-white shadow-lg shadow-green-500/30';
        break;
  ...
  const handleStart = () => {
    ...
    if (operation === 'enqueue') {
      ...
      if (isNaN(value) || !Number.isInteger(value) || enqueueValue.trim() === '') {
                ...
                <div className="bg-black/30 backdrop-blur-xl p-5 rounded-xl border border-white/10 shadow-xl">
                  {displayQueue.length > 0 ? (
                    ...
                    <div className="text-center text-gray-400 py-8">
                      ...
                      <p className="text-sm">Generate a queue to see preview</p>
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-5">
                ...
                <div className="bg-black/20 backdrop-blur-xl p-4 rounded-xl border border-white/10">
                  <label className="block text-sm font-medium text-gray-200 mb-3">
                    Queue Size: <span className="text-cyan-400 font-mono">{queueSize}</span>
                  </label>
                  <input
                    type="range"
                    min="3"
                    ...
                    style={{
                      background: `linear-gradient(to right, #06b6d4 0%, #06b6d4 ${((queueSize - 3) / 9) * 100}%, #374151 ${((queueSize - 3) / 9) * 100}%, #374151 100%)`
                    }}
                  />
                ...
                >
                  ...
                  <span className="font-medium">Generate New Queue</span>
                </motion.button>
              </div>
