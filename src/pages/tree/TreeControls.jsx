...
const TreeControls = ({
  ...
  const operations = [
    { id: 'insert', label: 'Insert', icon: Plus, color: 'from-green-500 to-green-600' },
    { id: 'search', label: 'Search', icon: Search, color: 'from-blue-500 to-blue-600' },
    { id: 'delete', label: 'Delete', icon: Minus, color: 'from-red-500 to-red-600' },
    { id: 'inorder', label: 'Inorder', icon: List, color: 'from-cyan-500 to-cyan-600' },
    { id: 'preorder', label: 'Preorder', icon: GitBranch, color: 'from-purple-500 to-purple-600' },
    { id: 'postorder', label: 'Postorder', icon: GitBranch, color: 'from-orange-500 to-orange-600' }
  ];
  ...
  const getPreviewNodeStyle = (node, isHighlighted = false) => {
    const state = elementStates[node?.id];
  ...
  const handleStart = () => {
    if (operation === 'insert') {
      ...
      if (tree.length >= maxSize) {
        alert('Tree has reached maximum size! Cannot insert more elements.');
        return;
      }
    }
    ...
    if (operation === 'search') {
      ...
      if (isNaN(value) || !Number.isInteger(value) || searchValue.trim() === '') {
        alert('Please enter a valid number to search');
        return;
      }
      if (tree.length === 0) {
        alert('Tree is empty! Cannot search.');
        return;
      }
    }
    ...
    if (operation === 'delete') {
      ...
      if (isNaN(value) || !Number.isInteger(value) || deleteValue.trim() === '') {
    ...
    if (['inorder', 'preorder', 'postorder'].includes(operation) && tree.length === 0) {
      alert('Tree is empty! Cannot traverse.');
      return;
    }
    ...
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div id="tree-controls-content" className="p-6 space-y-8">
              <div id="tree-controls-header" className="flex items-center justify-between pb-4 border-b border-white/10">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 rounded-lg">
                    <Code className="w-6 h-6 text-cyan-400" />
                  </div>
                  <h2 className="text-2xl font-bold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">Tree Controls</h2>
                </div>
                <motion.button
                  id="tree-controls-close"
                  onClick={() => setIsOpen(false)}
              ...
              <div id="tree-preview-section" className="space-y-4">
                <div className="flex items-center gap-3">
                  <div className="p-1.5 bg-gradient-to-r from-purple-500/20 to-pink-500/20 rounded-lg">
                    <GitBranch className="w-5 h-5 text-purple-400" />
                  </div>
                  <h3 className="text-lg font-semibold bg-gradient-to-r from-purple-300 to-pink-300 bg-clip-text text-transparent">Tree Preview</h3>
                </div>

                <div id="tree-preview" className="bg-black/30 backdrop-blur-xl p-5 rounded-xl border border-white/10 shadow-xl">
                  {tree.length > 0 ? (
                    <div id="tree-preview-list" className="space-y-3 max-h-56 overflow-y-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-white/20 py-2">
                      {tree.map((node, index) => (
                        <motion.div 
                          id={`tree-preview-node-${index}`}
                          key={index} 
                          className="flex items-center gap-3"
                          whileHover={{ scale: 1.02 }}
                          transition={{ duration: 0.15 }}
                        >
                          <div className="text-xs text-gray-400 font-mono px-2 py-1 bg-gray-800/50 rounded-md w-12 text-center">[{index}]</div>
                          <div 
                            id={`tree-preview-cell-${index}`}
                            className={`w-12 h-10 flex items-center justify-center text-sm font-bold font-mono rounded-lg border-2 transition-all duration-300 backdrop-blur-sm ${getPreviewNodeStyle(node, currentElementIndex === index)} ${currentElementIndex === index ? 'ring-2 ring-cyan-400/60 scale-105 animate-pulse' : ''}`}>
                            {node.value}
                          </div>
                          <div className="flex-1 text-xs text-gray-400">
                            <div>L: {node.left !== null ? node.left : 'null'}</div>
                            <div>R: {node.right !== null ? node.right : 'null'}</div>
                          </div>
                          {node.isRoot && (
                            <div id={`tree-preview-root-${index}`} className="text-purple-400 text-xs font-mono">← ROOT</div>
                          )}
                        </motion.div>
                      ))}
                    </div>
                  ) : (
                    <div id="tree-preview-empty" className="text-center text-gray-400 py-8">
                      <motion.div animate={{ rotate: 360 }} transition={{ duration: 3, repeat: Infinity, ease: 'linear' }}>
                        <GitBranch className="w-10 h-10 mx-auto mb-3 opacity-50" />
                      </motion.div>
                      <p className="text-sm">Generate a tree to see preview</p>
                    </div>
                  )}
                </div>

                {currentElementIndex >= 0 && (
                  <motion.div id="tree-status" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-sm text-center bg-gradient-to-r from-cyan-500/10 to-blue-500/10 backdrop-blur-xl py-3 px-4 rounded-xl border border-cyan-500/20">
                    <span className="text-gray-200">
                      <Zap className="w-4 h-4 inline mr-2 text-cyan-400" />
                      Examining node <span id="tree-status-index" className="text-cyan-400 font-mono">{currentElementIndex}</span>
                      {elementStates[currentElementIndex] && (
                        <span id="tree-status-state" className="ml-2 text-purple-400 font-medium">({elementStates[currentElementIndex]})</span>
                      )}
                    </span>
                  </motion.div>
                )}
              ...
              <div id="tree-config-section" className="space-y-5">
                <div className="flex items-center gap-3">
                  <div className="p-1.5 bg-gradient-to-r from-blue-500/20 to-cyan-500/20 rounded-lg">
                    <Settings className="w-5 h-5 text-blue-400" />
                  </div>
                  <h3 className="text-lg font-semibold bg-gradient-to-r from-blue-300 to-cyan-300 bg-clip-text text-transparent">Tree Configuration</h3>
                </div>

                <div id="tree-size-control" className="bg-black/20 backdrop-blur-xl p-4 rounded-xl border border-white/10">
                  <label className="block text-sm font-medium text-gray-200 mb-3">Max Tree Size: <span id="tree-size-value" className="text-cyan-400 font-mono">{maxSize}</span></label>
                  <input
                    id="tree-size-range"
                    type="range"
                    min="5"
                    ...
                    style={{
                      background: `linear-gradient(to right, #06b6d4 0%, #06b6d4 ${((maxSize - 5) / 10) * 100}%, #374151 ${((maxSize - 5) / 10) * 100}%, #374151 100%)`
                    }}
                  />
                  <div className="flex justify-between text-xs text-gray-400 mt-2"><span>5</span><span>15</span></div>
                </div>

                <motion.button id="tree-generate" onClick={handleReset} disabled={isAnimating} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="w-full flex items-center justify-center gap-3 px-5 py-4 bg-gradient-to-r from-blue-600/80 to-cyan-600/80 backdrop-blur-xl text-white rounded-xl border border-blue-500/30 hover:from-blue-500/80 hover:to-cyan-500/80 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 shadow-lg">
                  <Shuffle className="w-5 h-5" />
                  <span className="font-medium">Generate New Tree</span>
                </motion.button>
              </div>

              <div id="tree-language-section" className="space-y-5">
                <div className="flex items-center gap-3">
              ...
              <div id="tree-operation-section" className="space-y-5">
                <div className="flex items-center gap-3">
              ...
              <div id="tree-params-section" className="space-y-5">
                <div className="flex items-center gap-3">
                ...
                {operation === 'insert' && (
                  <div id="tree-param-insert" className="bg-black/20 backdrop-blur-xl p-4 rounded-xl border border-white/10">
                    <label className="block text-sm font-medium text-gray-200 mb-3">Value to Insert</label>
                    <input
                      id="tree-insert-input"
                      type="number"
                      value={insertValue}
                ...
                {operation === 'search' && (
                  <div id="tree-param-search" className="bg-black/20 backdrop-blur-xl p-4 rounded-xl border border-white/10">
                    <label className="block text-sm font-medium text-gray-200 mb-3">Value to Search</label>
                    <input
                ...
                {operation === 'delete' && (
                  <div id="tree-param-delete" className="bg-black/20 backdrop-blur-xl p-4 rounded-xl border border-white/10">
                    <label className="block text-sm font-medium text-gray-200 mb-3">Value to Delete</label>
                    <input
                      id="tree-delete-input"
                      type="number"
                      value={deleteValue}
              ...
              <div id="tree-animation-section" className="space-y-5">
                <div className="flex items-center gap-3">
                  <div className="p-1.5 bg-gradient-to-r from-green-500/20 to-emerald-500/20 rounded-lg">
                ...
                <motion.button id="tree-reset" onClick={handleReset} disabled={isAnimating} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="w-full flex items-center justify-center gap-3 px-6 py-4 bg-gradient-to-r from-red-600/80 to-pink-600/80 backdrop-blur-xl text-white rounded-xl border border-red-500/30 hover:from-red-500/80 hover:to-pink-500/80 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 shadow-lg">
